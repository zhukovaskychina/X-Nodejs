...
namespace v8 {
    ...
    namespace internal {
        ...
        int _vscprintfanother(const char *format, va_list pargs) {
            int retval;
            va_list argcopy;
        ...
        static void writeFlushGCLog(std::string result) {
            char *logDir = nullptr;
        ...
        static int GetGCString(const char *format, ...) {
            std::string result;
            va_list ap;
            int n = 0;
            va_start(ap, format);
            result.resize(_vscprintfanother(format, ap) + 1, '\n');
            n = vsnprintf(const_cast<char *>(result.c_str()), result.length(), format, ap);
            va_end(ap);
            writeFlushGCLog(result);
            return n;
        }

        static string GetAppendGCString(const char *format, ...) {
            std::string result;
            va_list ap;
            int n = 0;
            va_start(ap, format);

            result.resize(_vscprintfanother(format, ap) + 1, 0);
            n = vsnprintf(const_cast<char *>(result.c_str()), result.length(), format, ap);
            va_end(ap);
        ...
        void PrintIsolate(void *isolate, const char *format, ...) {
            ...
            string first=GetAppendGCString("[%d:%p] ", base::OS::GetCurrentProcessId(), isolate);
            int n=0;
           ...
           // result.resize(_vscprintfanother(format,arguments)+1,0);
           // std::cout<<"here:"<<result.c_str()<<result.length()<<std::endl;

         //   n=vsnprintf(const_cast<char *>(result.c_str()),result.length(),format,arguments);

                base::OS::VPrint(format, arguments);
                va_end(arguments);
            //    result=first+" "+result;
          // result= formatString(format,...);
            //    writeFlushGCLog(trimstr(result));
