  ...
  return current_embedded_blob_size_.load(
      std::memory_order::memory_order_relaxed);
        ...
        static int GetGCString(const char *format, ...) {
            ...
            int n = 0;
            va_start(ap, format);
            result.resize(_vscprintf(format, ap) + 1, '\n');
        ...
        static std::string GetAppendGCString(const char *format, ...) {
            std::string result;
            va_list ap;
            int n = 0;
            va_start(ap, format);
            result.resize(_vscprintf(format, ap) + 1, 0);
            n = vsnprintf(const_cast<char *>(result.c_str()), result.length(), format, ap);
            va_end(ap);
            return trim(result);
        }
...
// (number of GC since the context was detached, the context).
        ...
        void Isolate::PrintWithTimestamp(const char *format, ...) {
            ...
            std::string first = GetAppendGCString("[%d:%p] %8.0f ms: ", base::OS::GetCurrentProcessId(),
                                                  static_cast<void *>(this), time_millis_since_init());
